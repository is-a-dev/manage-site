...
export async function GET({ url, cookies }) {
    ...
    let token = await fetch(`https://github.com/login/oauth/access_token?client_id=${env.GITHUB_CLIENT_ID}&client_secret=${env.GITHUB_CLIENT_SECRET}&code=${query}`, {
        method: "POST",
        headers: {
    ...
    if (!token.access_token) return new Response('The access token was not found in the response', { status: 400 })
